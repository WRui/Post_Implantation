...
print OUT "Sample\tTotalCpGs(1X)\tTotalUnique(1X)\tMeanCov(1X)\tMethRatio(1X)\tMethReadRatio(1X)\tTotalCpGs(3X)\tTotalUnique(3X)\tMeanCov(3X)\tMethRatio(3X)\tMethReadRatio(3X)\tTotalCpGs(5X)\tTotalUnique(5X)\tMeanCov(5X)\tMethRatio(5X)\tMethReadRatio(5X)\tTotalCpGs(10X)\tTotalUnique(10X)\tMeanCov(10X)\tMethRatio(10X)\tMethReadRatio(10X)\n";
...
#	print "$line\n";
	...
	for($j=0;$j<$length;$j++){
		if($words[4]>=$depth[$j] && $words[9]=~/CpG/){
			#print "@words\n";
			$sum[$j]+=$words[4];
			#print "total\t$words[4]\t";
			$num[$j]++;
